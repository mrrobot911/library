    ...
    <header className="flex relative items-center justify-between max-w-[1440px] h-[90px] bg-[#0C0C0E]">
        ...
        <div className="flex flex-row px-5 items-center justify-center">
            <nav className="hidden md:flex md:justify-between md:gap-[40px]">
                ...
                { auth ? <p className="text-[15px] text-[#BB945F] p-[5px] bg-white rounded-[50%] cursor-pointer ml-[40px] uppercase"onClick={()=>setMenuUser(!menuUser)}>{`${userData.firstname[0]}${userData.lastname[0]}`}</p> : <a className="block ml-[40px] cursor-pointer duration-200 fill-white hover:fill-[#BB945F]"onClick={()=>setMenuUser(!menuUser)}><svg width="28" height="28" viewBox="0 0 28 28" xmlns="http://www.w3.org/2000/svg">
